...
public class MailSendService {
	...
	// 비밀번호 인증코드 발급
	public void sendAutoMailFindPw(String email) {
		
		String authKey = getKey(6);
				
		try {
            MailUtils sendMail = new MailUtils(mailSender);
            sendMail.setSubject("Test Study");
            sendMail.setText(new StringBuffer().append("<h1>[ Test입니다 ]</h1><hr>")					
					.append("<h2 style='width: 170px; background-color: green; color: black;'> 인증번호 : Test : " + authKey + "</h2>")
					.toString());
